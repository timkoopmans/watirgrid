...
The Ruby core library for Rinda is broken as it incorrectly uses the each method on a string. Without resorting to monkey patches, you can avoid this error in watirgrid by not using the DRb broadcast methods used in a lookup. This happens when you *don't* specify a controller URI. To avoid this:

1. Specify the controller URI if starting a provider from the command line
...
2. Specify the controller URI if staring a provider programatically

```
provider = Provider.new(:driver => 'webdriver', :controller_uri => 'druby://203.51.48.187:11235')
```

3. And always specify the controller URI when instantiating your grid!
